...
post_pred_newsubj <- function(betaSamp, errVarSamp, Sigma,
    ...
    #### vectorizing the Zij arrays ####
    vecZij_list_test <- lapply(zlist_test,
...
post_pred_existingsubj <- function(betaSamp, errVarSamp, Lkron,
                                   xlist_test, zlist_test, mis, nom.level) {
    ...
    vecXij_test <- lapply(xlist_test, function(foo) {as.vector(foo)})
    ...
    for (gg in 1:n_test) {
        yhat <- list()
        for (tt in 1:niter) {
            bi <- rnorm(ncol(Lkron), 0, sqrt(errVarSamp))
