post_pred_newsubj <- function(object, kdims, R_list, S_list,
    ...
    for(ij in 1:N_test) {
        z_tilde_mode1 <- S_list[[1]] %*% tcrossprod(mode_matricize(zlist_test[[ij]], n = 1), Skron_not1)
    ...
    for (gg in 1:n_test) {
        ...
        resids[[gg]] <- ylist_test[[gg]] - preds_mean[[gg]]
    ...
    result <- list(y_pred = preds, yhat_samples = yhat_samples,
                   qlower = qlower, qupper = qupper)
    result
}


post_pred_existingsubj <- function(object,
                                   R_list, S_list, kdims,
