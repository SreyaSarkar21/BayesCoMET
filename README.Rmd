...
# 
# beta_df <- data.frame(
#   postsamp = c(res$betaSamp[, 73], res$betaSamp[, 1]),
#   betaidx  = factor(rep(idx, each = niter), levels = idx))
# 
# bsumm_df <- data.frame(
#   q025 = c(quantile(res$betaSamp[, 73], 0.025), quantile(res$betaSamp[, 1], 0.025)),
#   q50 = c(median(res$betaSamp[, 73]), median(res$betaSamp[, 1])),
#   q975 = c(quantile(res$betaSamp[, 73], 0.975), quantile(res$betaSamp[, 1], 0.975)),
#   true = c(vecB_true[73], vecB_true[1]),
#   betaidx = factor(idx, levels = idx))
# 
