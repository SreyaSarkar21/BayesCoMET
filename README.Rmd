...
```{r visualize_betaSamps, echo = FALSE, message=FALSE, warning=FALSE, fig.width=8, fig.height=3.2}
library(dplyr)
library(ggplot2)
...
bsumm_df <- data.frame(q025 = c(quantile(res$betaSamp[, 73], 0.025), quantile(res$betaSamp[, 1], 0.025)),
                       ...
                       q975 = c(quantile(res$betaSamp[, 73], 0.025), quantile(res$betaSamp[, 1], 0.025)),
...
ggplot(filter(beta_df, betaidx == bname), aes(x = postsamp)) +
    geom_histogram(aes(y = after_stat(density)),
                   bins = 25, fill = "grey80", color = "black") +
    geom_vline(data = filter(bsumm_df, betaidx == bname),
               aes(xintercept = q025, linetype = "95% CI", color = "95% CI"),
               linewidth = 0.8) +
    geom_vline(data = filter(bsumm_df, betaidx == bname),
               aes(xintercept = q975, linetype = "95% CI", color = "95% CI"),
               linewidth = 0.8) +
    geom_vline(data = filter(bsumm_df, betaidx == bname),
               aes(xintercept = q50, linetype = "Posterior median", color = "Posterior median"),
               linewidth = 0.9) +
    geom_vline(data = filter(bsumm_df, betaidx == bname),
               aes(xintercept = true, linetype = "True value", color = "True value"),
               linewidth = 0.9) +
